...
const Project = ({ repo }: Props) => {
  ...
  return (
    <div className="w-full flex flex-col border hover:border-primary rounded-lg p-3 transition-colors relative group">
      <Link target="_blank" href={link} className="w-6 aspect-square rounded-full bg-primary text-primary-foreground absolute -top-2.5 -right-2.5 z-10 hidden group-hover:flex items-center justify-center">
        <MdOpenInNew size={14} />
      </Link>
            ...
            <div className='relative w-full h-fit flex items-center justify-start'>
              {
                top_tags.map((tag, i, arr) =>
                  <Fragment key={tag + '-' + i}>
                    <span className="text-xs w-fit text-muted-foreground">{tag}</span>
